...
describe("cannotBeValidDate", () =>
{
    ...
    {
        let validator = new CannotMatchValidator(/^[0-9]+$/, null, null, ValidationLevel.error, ValidationContext.default, 0);
    ...
    {
        let validator = new CannotMatchValidator(/^[a-z]+$/, null, null, ValidationLevel.error, ValidationContext.default, 0);

        assert.equal(validator.message, "Value cannot match /^[a-z]+$/.");
        assert.equal(validator.messageKey, "CannotMatch");
    ...
    {
        let validator = new CannotMatchValidator(/^[0-9]+$/, "Test message.", "Test message key", ValidationLevel.info, "test context", 75);
